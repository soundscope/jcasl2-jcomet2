...
public class JaCasl2Test {
    ...
    public void bug3() throws Exception {
        Path casPath = Paths.get(JaCasl2Test.class.getResource("/bug3.cas").toURI());
    ...
    public void bug5() throws Exception {
      Path casPath = Paths.get(JaCasl2Test.class.getResource("/bug5.cas").toURI());
      Path inputPath = Paths.get(tmpDir.toString(), casPath.getFileName().toString());
      Files.copy(casPath, inputPath, StandardCopyOption.REPLACE_EXISTING);
      ...
      JaCasl2.main(new String[]{inputPath.toString()});
  		assertTrue(err.toString().contains("JaCasl2 Error: The length of label must not exceed 8 characters."));
  		assertTrue(err.toString().contains("Line 4: LABEL1234 NOP"));
    }
    
    @Test
    public void bug6() throws Exception {
      Path casPath = Paths.get(JaCasl2Test.class.getResource("/bug6.cas").toURI());
      Path inputPath = Paths.get(tmpDir.toString(), casPath.getFileName().toString());
      Files.copy(casPath, inputPath, StandardCopyOption.REPLACE_EXISTING);
      ByteArrayOutputStream err = new ByteArrayOutputStream();
  		System.setErr(new PrintStream(err));
      ...
      JaCasl2.main(new String[]{inputPath.toString()});
  		assertTrue(err.toString().contains("JaCasl2 Error: Undefined label \"LABEL123\" was found."));
  		assertTrue(err.toString().contains("Line 4:  JUMP LABEL123"));
    }
    
    @Test
    public void bug7() throws Exception {
      Path casPath = Paths.get(JaCasl2Test.class.getResource("/bug7.cas").toURI());
      Path inputPath = Paths.get(tmpDir.toString(), casPath.getFileName().toString());
      Files.copy(casPath, inputPath, StandardCopyOption.REPLACE_EXISTING);
      ByteArrayOutputStream err = new ByteArrayOutputStream();
  		System.setErr(new PrintStream(err));
      ...
      JaCasl2.main(new String[]{inputPath.toString()});
  		assertTrue(err.toString().contains("JaCasl2 Error: Invalid operation is found."));
  		assertTrue(err.toString().contains("Line 3:  LAD G1,1"));
    }

    private void test(String target) throws Exception {
        Path casPath = Paths.get(JaCasl2Test.class.getResource("/enshud/" + target + ".cas").toURI());
        Path comPath = Paths.get(JaCasl2Test.class.getResource("/enshud/" + target + ".com").toURI());
